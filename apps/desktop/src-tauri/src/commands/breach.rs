...
use keepassex_core::breach::{check_password_offline, sha1_hex};
use serde::Serialize;
use tauri::State;
...
/// Check all vault passwords against HIBP (online k-anonymity or offline)
#[tauri::command]
pub async fn check_vault_breaches(
    online: bool,
    state: State<'_, AppState>,
) -> Result<VaultBreachReport, String> {
    // Collect all data BEFORE any await point — avoids Send issues with RwLock
    ...
    // State lock is released here — safe to await
    ...
    if online {
        ...
        for (uuid, title, password) in &entries {
            match keepassex_core::breach::check_password_hibp(password).await {
                Ok(r) if r.is_breached => {
                    results.push(BreachCheckResult {
                        entry_uuid: uuid.clone(),
                        entry_title: title.clone(),
                        is_breached: true,
                        breach_count: r.breach_count,
                        hash_prefix: r.password_hash_prefix,
                    });
                }
                _ => {}
            }
        ...
        // Offline check against common passwords list
        for (uuid, title, password) in &entries {
            if check_password_offline(password) {
